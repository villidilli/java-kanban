...
public class HttpTaskManager extends FileBackedTasksManager {
	...
	public HttpTaskManager(int serverPort) {
		try {
			gson = GsonConfig.getGsonTaskConfig();
			kvClient = new KVTaskClient(serverPort);
			load();
	...
	private void addToHistory(ArrayList<Integer> historyIDs) {
		for (Integer id : historyIDs) {
			if (tasks.get(id) != null) historyManager.add(tasks.get(id));
			if (subTasks.get(id) != null) historyManager.add(subTasks.get(id));
			if (epics.get(id) != null) historyManager.add(epics.get(id));
		}
	}
